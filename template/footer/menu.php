...
<div class="flex flex--column container">
    ...
    <nav class="flex flex--column nav">
        <?php foreach (wp_get_nav_menu_items($menu -> term_id) as $item): ?>
        <?php $target = $item -> target; ?>
        <?php $xfn = $item -> xfn; ?>
        <a href="<?php echo $item -> url; ?>"<?php if ($xfn): ?> rel="<?php echo $xfn; ?>"<?php endif; ?><?php if ($target): ?> target="<?php echo $target; ?>"<?php endif; ?> class="nav__item"><?php echo $item -> title; ?></a>
        <?php endforeach; ?>
    </nav>
</div>
<!-- / Footer Menu -->
